...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer**The columns that exist are OBJECTID, ZIPCODE2, EXISTING, ZIPCODE1, Shape_Leng, Shape_Area, geometry 
...
**Answer** In the output, I see what type of variables each column name is. I also see the heads of each column. There is a new datatype that I just learned about in the output, geometry. You can see the different longitude and latitude points in Maryland, for all of these different locations. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We can now see the latitude and longitude points plotted on a grid, over the state of maryland. Each shape on the map is a different zip code. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This output is just loading the libraries, and installing the 'tigris' package,then opening it. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** In the output below, we can see the data for the year 2024 and instead of just Maryland, we can see all USA counties. I looked up what these new column names were because I didn't know, they are geographic / census metadata columns that go with the data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Using the filtering codes, I see that the output is now only going to show the 24 counties or jurisdictions that are in Maryland. This will help us to easier narrow down and check out our data. I also looked up that STATEFP (24) is the FIPS code for Maryland. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** In the output, we can see the different shapes of all of the Maryland counties, printed on a latitude-longitude map of Maryland. This way we can clearly see the different counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The output now shoes different and even more specific boundaries in Maryland. The data set is added within the counties, displaying them on the map. They are layered on the map below. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**This code and output is joining the two datasets, Maryland zip spatial data and the March 2023 foreclosure data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**This output is using the March 2023 data, layering it on the original counties. It also includes a legend and color scheme which shows the least amount of calls (darker colors) to the most amount of calls (lighter colors) 
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** The output is just loading the census api key, preparing it to load the data. It also loads tidycensus 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This codeblock is retrieving the acs data data using tidycensus it is also creating variables that will later be filled. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The output has retrieved the data, and is now arranging it into the datatable showing the first ten features. Looking up the name of this type of table, The output is a sf (simple features) tibble, which combines tabular data with geometry. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This output is simply loading the two data tables that we are dealing with, allowing us to be able to view them below. It also tells us the different column names and what type of variables they are. I see there are some discrepancies in lower and upper case lettering, the date is also a chr. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This output has cleaned the population table, renaming the columns and changing the titlecases. Now, the tables will be ready for joining. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**This output is joining the tables, again in a simple feature table. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code is calculating percentages for forclosure notices per 1000 population. It also then sorts it from highest to lowest. There is now a new dataframe with this information. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have our final output which is showing the color coded map, with lighter colors at a higher rate and darker colors at a lower rate. We also now have labels for each county, making it a lot easier to read and see the data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This output is changing the color scale to a more variety of different colors which helps make the map easier to read and helps to distinguish the different counties. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** This output changed the colors a bit more, to see the differences even more clearly. 
