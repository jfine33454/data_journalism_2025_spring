...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are a few problems with the data, first there are several duplicates we can see like "Walden Sierra" 
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer**Looking at the first few pages of data, I see that the grantee and grantor columns are flipped. I also see that the cleaned data, the grantee column is less specific. This is probably because more departments were grouped together.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer**There are a few changes I can see on the first pages of the data. The most noticeable being the consolidation of all the different "walden sierra" entries into one, under one name. Something else that was noticed was the WALTER'S ART MUSEUM and THE WALTERS ART MUSEUM were not grouped together, this makes me wonder, how do you deal with apostrophes? Also, everything is now capitalized. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No, not really
