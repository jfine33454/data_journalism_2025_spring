...
**Task** Load some [Maryland state government payments data](https://opendata.maryland.gov/Budget/State-of-Maryland-Payments-Data-FY2008-to-FY2024/7syw-q4cy) by running the following code. We'll use the guess_max() function as an argument to use the first 10 rows to set the data type. What does the first line of the red Warning message that prints out when you load the data say? What happens when you follow its instructions? Answer below. **Answer** The message says, Warning: One or more parsing issues, call `problems()` on your data frame for details, e.g.:When I looked up the instructions, I learned two things happen. First, the Use `spec()`line, retrieves the full column specification for this data. The second instructional line, Specify the column types or set `show_col_types = FALSE` leads to quieting or getting rid of the message altogether. 
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** It shows an error message saying "Error in clean_names(md_grants_loans) : 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** The Vendor Name is filed as a <chr> or character. The amount is filed as a <dbl> and the date column is filed as a <chr> as well. 
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer** There are 553, this is probably because the zip code column is classfied as a number however, it really isn't a number it's a specific code. 
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** Below, I see that we are counting how many records their are for each fiscal year and category and then sorting it by fiscal year. Something that kind of jumps out at me is that there is no is no loan data for 2009 there are a few records where the Category is NA - that data is missing. There are a few inconsistent values.  
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** I see that there are some unusual out-liars There are two grants for less than $100, which is weird. 
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** It got rid of capital letters and replaced spaces with underscores. 
```{r}
# cleaning function
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** This function cleans up the function names and then makes sure that there is consistent capitalization using the mutate statement. It is overwriting the columns. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** As many as 58 duplicates are possible 
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** The new dataframe 19,453 rows 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** It gets rid of everything after the first five digits. 
