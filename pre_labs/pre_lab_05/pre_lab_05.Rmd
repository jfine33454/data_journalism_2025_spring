...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** I see that there are column headers, the headers shouldn't be headers. 
...
fentanyl_deaths <- read_csv("tabula-Annual_2020_Drug_Intox_Report.csv") %>%
  clean_names()
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer**The problem that remains is that we do not have any NAMES for the headers. 
...
**Task** Run the codeblock below. What else do we need to do? **Answer** There are a bunch of periods in the jurisdiction column, it will be easier to filter once we remove them. 
