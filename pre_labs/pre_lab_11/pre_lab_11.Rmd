...
**Task** Run the following code and describe the data frame it outputs. **Answer** The output is a data frame with 15 columns, that contain the names of different files, their links, name information, file types, year, and file content. There are 499 entries or rows for this frame. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** I see that this is making every word lowercase, it also removes any special characters. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** I don't think it is that interesting, it shows which words are used the most in the alerts dataset. I see the top words are "the" "of" and "to" these are all pretty common words so it doesn't surprise me to much. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** This table is a bit more interesting because the words are more unique. It makes the analysis a lot more interesting. 
...
**Task** Run the following code and describe the results. **Answer** We are now comparing both datasets for 2022 and 2023 and the top ten words that are used. I see that the top word was discipline, then health, then patient. 
...
**Task** Run the following code and describe the results. **Answer** We are now looking at two word phrases. The result is a dataframe showing the top ten two word phrases that were written in 2023. The top result is health occ and the second is md code. 
...
**Task** Run the following code and describe the results. **Answer** The results of this code is creating the top ten 2 word phrases in 2024. I see that in 2024 the top two word phrases are health occ and summary suspension. The percentages are still pretty small. I notice that for 2023 and 2024, the same two word phrase is used the most. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** This results of this dataframe show the different sentiments of different words. Looking through the list a word I found that could be counter-intuitive is "skinny" its negative, but I guess in different circumstances could be positive. "Joke" could also be seen as positive. 
