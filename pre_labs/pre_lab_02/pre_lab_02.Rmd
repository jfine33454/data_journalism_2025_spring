...
**Answer** It is important because it keeps the old data secure, without erasing it. It also helps to clarify the different variables making the data a lot easier to read. 
...
**Answer:** Instead of the catagory being a <chr> it says <date> I learned that this means that it is now an actual date column instead of a character. 
...
**Answer** Instead of a single date, the column is now organized by complete months, the first of one month to the first of the next. 
...
**Answer** The code is adding two new columns to the table, total_votes and pct_moore. 
...
**Answer** It used the arrange function and arranged the data in a easier to read and more concise way. It makes it so the highest/best percentages are at the top of the list.  
...
**Answer** I think this data is interesting, but it depends on what sort of data you are looking for/what questions you are trying to solve. This data will show Moore's weakest areas and his lowest percentages while the other set will show his strongest. 
...
**Answer** I used chatgpt to find this answer, I asked it "what are some tidyverse functions similar to str_to_upper.
str_to_lower(string, locale = "en")- Converts all characters of a string to lowercase. 
str_to_title(string, locale = "en")- Converts strings to title case, meaning the first letter of each word is capitalized.
...
**Answer** I think that the mutate statement is making a new data table that is the same as the last but adding a new column for the "state" category. 
...
**Answer** Yes, these totals make sense to me because MD still has the largest total amount of expenses.The NA row is confusing me and may confuse the rest of the table however, hopefully it is not messing up the table. 
...
1.  Which month in 2022 had the highest number of expenditures? 

2.  Which state had the highest total number? How much higher was it than the other states, the second highest state? 

3.  Which state had the lowest number of expenditures? 
