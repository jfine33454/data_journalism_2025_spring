...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** Thr first line of code is creating a new dataframe, naming it baltco_911_calls_by_month. It will have the new month column. The second line is actually creating the new month column by only taking the month name. the label = TRUE part makes sure that the month column is a chr or name column instead of a number or date. The next line groups all the data by month so there will be 12 observations, one for each month. The next line is counting how many 911 calls are in each month, and creating the total_calls column. Lastly, the last line is arranging or sorting the results by amount of calls, so the months with the most calls appear on top.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It is a "blank canvas" this will hold our chart. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** In the space below, there is a basic bar chart describing the data. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** We used the reorder() function to reorder the counties from highets to lowest by total_loans. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now, we flipped the hart from a vertical bar chart to a horizontal one using the coord_flip() function. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** We are changing the theme to remove the gray background. Using the theme_minimal() function, we can make the chart look a bit prettier. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer**After running the code, I see the background changed again, and we added a new theme called the economists theme. The new package we just installed allows us to choose new themes. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** We changed the axis labels to make them a bit more descriptive and clear. We added the source in the corner and changed "month" and "total calls."                                         
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We are now working towards creating another chart type, a line chart. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We now put the date on the X axis and the total on the y axis we can now see after running the code, if there are any patterns in the data. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We added tick marks using a function called scale_x_date() in order to see when specifically months with calls spike. There are now date breaks. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**This step made the graph much easier to read. We changed month abbreviations to avoid overlapping. 
