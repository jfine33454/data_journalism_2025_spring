...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** Yes, all of the datasets have the same number of columns, all with the same names. 
...
```{r}
# bind_rows with list
county_voters_combined <- bind_rows(list(
  county_voters_2016,
  county_voters_2018,
  county_voters_2020,
  county_voters_2022,
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows, there should be 24.
...
**Task** Examine the combined data Even without the nrow, there is still 24 rows!
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** It says it is joining the data by (COUNTY). It also says in the book, that the columns are also no longer just limited to three outputs. 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** The county with the highest percentage is Prince George's County is around 61%. The percentage of democrats in that county is around 78%. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard county has the highest percent of an Asian population, around 19% and the number of democrats is around 52%. The percents are both significantly lower than the previous task, meaning that the jurisdictions with a high amount of democrats, have small asian populations. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** It is a <dbl> or a numeric column. This shouldn't be because they are codes. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** Now it is a character column 
