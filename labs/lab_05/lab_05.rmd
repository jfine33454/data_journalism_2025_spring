...
```{r}
 od_log <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |>
  clean_names() |>
 rename(od_date = x1, time = x2, case_number = x3, event_type = x4, location = x5)
od_log$od_date <- as.Date(od_log$od_date, format = "%m/%d/%y")
```
...
A1.I see that in 2022, there are two dates that had 46 calls. 7/14/2022 and 10/4/2022. I searched through the data and found that there were zero dates with zero calls. I also figured that because there are 366 observations, and 365 days in a year so each day there was a call. 

```{r}
library(tidyverse)

calls_per_day <- od_log |>
  group_by(od_date) |>
  summarise(num_calls = n()) |>
  arrange(desc(num_calls))
most_calls <- calls_per_day |>
  slice_max(num_calls, n = 1)
...
A2.I see that most calls occurred on Saturdays, and the least amount of calls occured on Thursdays. 
...
od_log <- od_log |>
  mutate(day_of_week = wday(od_date, label = TRUE, abbr = FALSE)) %>%
  group_by(day_of_week) |>
  mutate(total_calls = n()) |>
  ungroup() |>
  mutate(percent_calls = total_calls / nrow(od_log) * 100)

od_log |>
  distinct(day_of_week, total_calls, percent_calls) |>
  arrange(match(as.character(day_of_week), c("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday")))
```
...
A3. I see that the location with the most amount of calls was 4540 SILVER SPRING RD	and PC 02; 6424 WINDSOR MILL RD. I looked more into these locations and was a bit confused, because they both looked like pretty residential areas. While there were some stores nearby, that could have been the cause of the calls, it made me less confident in the data. 
...
od_log |>
  count(location, name = "total_calls") |>
...
A4. A question I have, is are the locations with the most calls accurate? A story I would potentially do is maybe try and find a reason why most overdoses happen on Sundays.
