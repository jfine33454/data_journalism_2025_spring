...
library(tidyverse)
install.packages("janitor")   # only run ONCE — not every time
library(janitor)

# load data
expenditures <- read_csv("data/maryland_expenses.csv")

# Clean column names
cleaned_expenditures <- expenditures |>
  clean_names()
# Convert the expenditure_date column to Date
cleaned_expenditures <- cleaned_expenditures |>
  mutate(expenditure_date = as.Date(expenditure_date, format = "%m/%d/%Y"))
glimpse(cleaned_expenditures)
...
**A1.** The top commitees that spent the most were Jain Ashwani Friends of	, Franchot Anderson-Walker for Maryland Slate	, Franchot Peter Friends Of	, Casa in Action PAC	I see that some of the names are cut off. I tried many times fixing it by myself, something must have gotten cut off when importing from openrefine? Not sure, will ask in class. 

```{r}
media_expenses <- cleaned_expenditures |>
  filter(!is.na(payee_name) & expense_category == "Media")
write_csv(media_expenses, "data/media_expenses.csv") 
library(tidyverse)
media_expenses <- read_csv("/Users/jfine/Desktop/JOUR 472/GitHub/data_journalism_2025_spring/labs/lab_04/media-expenses-csv.csv")
library(tidyverse)

facebook_spending <- media_expenses |>
  filter(str_detect(payee_clean, regex("Facebook|Meta", ignore_case = TRUE))) |>
...
**A2.** I looked up some of the top spenders to see what they care most about. Ashwani Jain cares a lot about free education and canceling loans for college students. He is also passionate about raising minimum wage and lowering medicare. The second highest committee is Franchot Anderson-Walker. Walker has strong opinions on more local policy issues and fixing her community. Franchot Peter is a big supporter of environmental changes and looks to support public education. I looked into facebooks ad library and searched Maryland. I saw the Maryland Department of Health was a top spender along with the republican state. 
...
**A3.** Looking at the list, I know that Wes Moore ultimately won the election. He has two committees Wes Moore for Maryland and Moore-Miller for Maryland (which is with his running mate). It is interesting that he did not spend the most or even was on the list, but won the election. 
...
**A4.** I was surprised to see that the data was a little different. Wes Moore spent the most on online advertising but he barely spent any on facebook. I also see that Franco Anderson-Walker spent about half of his online funds on facebook. 

```{r}
media_expenses_only <- media_expenses[media_expenses$expense_category == "Media", ]
media_by_committee <- aggregate(amount ~ committee_name, data = media_expenses_only, sum)
media_by_committee <- media_by_committee[order(-media_by_committee$amount), ]
print(media_by_committee)
...
**A5** I think an interesting story could be about how Wes Moore spent the most amount of money with online advertising yet spent little money on facebook, does he not like facebook? Is there a reason he didn't advertise with it? 
